...
class TimeSeriesDataset(Dataset):
    """
    Time Series Dataset class. Full Implementation in utils/timeseriesdataset.py
    """
...
def create_instance(item, augment, shared_counter, lock):
    ...
    with lock:
        ...
        print(f"Instances processed: {shared_counter.value}", end='\r') # print for updates
...
if __name__ =="__main__":
    ...
    # for inference on a single dataset this needs to be a concatenated .parquet file, otherwise directory for training
    ROOT_PATH = "<directory for training or .parquet file for inference>" # User Variable

    if os.path.isdir(ROOT_PATH):

        # after running concat these data files should be present, Run concat first
        data_paths = [
        ...
        data_paths = [p for p in data_paths if os.path.isfile(p)] # needs to be present
        ...
        create_instances_parallel(train_data, augment=True, save_path=os.path.join(ROOT_PATH, "train_instances.pkl")) # only augment training data
    ...
    else: # create a single data pickle file if ROOT_PATH is not a directory

        directory, filename = os.path.split(ROOT_PATH)
        SAVE_PATH = os.path.join(directory, os.path.splitext(filename)[0] + ".pkl")
        print("Saving to:", SAVE_PATH)
        groups = read_groups([ROOT_PATH])
